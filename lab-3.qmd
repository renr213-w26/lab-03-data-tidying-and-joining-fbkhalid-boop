...
-   Go to the course organization at [github.com/renr213](https://github.com/renr213) organization on GitHub. Click on the repo with the prefix **lab-3**. It contains the starter documents you need to complete the lab.
...
The country_inflation dataset has 44 rows and 30 columns. Each row represents a specific country, and each column (other than the country name header) represents a specific year containing the annual inflation rate for that country.
...
country_inflation |>
  ...
  select(country, `2021`)
```
The top three countries with the highest inflation rates in 2021 are Argentina, Turkey, and Brazil. Compared to the United States in that same year which is at 4.7%, these three nations experienced much higher inflation rates, representing extreme economic volatility compared to the US.
### Question 3
...
Based on the pipeline output, the country whose inflation change is the largest over this time period is New Zealand with a ratio of 3.059. Because this ratio is greater than 1, it indicates that the inflation significantly increased between 1993 and 2021.
...
dim(country_inflation_longer)
```
The resulting pivoted data frame now has 1276 rows and 3 columns. By pivoting, we moved the years from column headers into a single variable, making the data much more tidy and suitable for visualization.
...
```
The highest inflation rate observed between 1993 and 2021 was in 1994 of 2075.888% in Brazil.
```{r}

country_inflation_longer |>
  filter(annual_inflation == min(annual_inflation, na.rm = TRUE))
```
The lowest inflation rate observed between 1993 and 2021 was in Ireland in the year 2009.
...
countries_of_interest <- c("United States", "Canada", "United Kingdom", "New Zealand", "Australia")
```
...
These are the five eyes countries and I think it would be interesting to how these intelligence sharing countries also share inflation and economic indicators.
...
coi_inflation <- country_inflation_longer |>
...
```{r}
#| label: q7-visualize-inflation

ggplot(coi_inflation, aes(x = year, y = annual_inflation, color = country, shape = country)) +
  geom_point(alpha = 0.7) +
  geom_line(linewidth = 1) +
  scale_color_viridis_d() +
  theme_minimal() +
  labs(
    title = "Annual Inflation Rates (1993-2021)",
    subtitle = "Comparing the economies of the Five Eyes Alliance",
    x = "Year",
...
```
Based on the plot, inflation rates across these major economies remained relatively predictable and clustered tightly together for most of the timeline between 1993 and 2020. I notice multiple sharp, synchronized upticks starting right around major events. This is largely unsurprising, as it visually captures economic disasters like the dot com bubble, the 2008 recession or the global economic impact caused by the COVID-19 pandemic.
...
The us_inflation dataset contains 132 rows and 4 columns. The variables represent tracking information like the year, the inflation rate, and a numeric ID corresponding to a specific CPI division.
...
The cpi_divisions dataset has 12 rows and 2 columns. It acts as a lookup table consisting of a division ID variable and a text description of what that division category actually measures.
...
```{r}
us_inflation_joined <- us_inflation |>
  left_join(cpi_divisions, by = join_by(cpi_division_id == id)) 

dim(us_inflation_joined)
names(us_inflation_joined)
```

I used a left_join() to retain all observations from the us_inflation data while appending the text descriptions from cpi_divisions. Because the ID column names differed between the datasets, I used the join_by() argument to explicitly map them. The resulting dataset has 132 rows, 5 columns, and successfully includes the human-readable descriptions.
### Question 9
...
I chose these five divisions because they represent essential, core living expenses for the average Canadian household, which makes their inflation trends highly relevant to daily life.
...
```{r}
us_inf_filtered <- us_inflation_joined |>
...
ggplot(us_inf_filtered, aes(x = year, y = annual_inflation, color = description, shape = description)) +
  geom_point(alpha = 0.8) +
  geom_line(linewidth = 1) +
  theme_light() +
  scale_color_brewer(palette = "Dark2") +
  labs(
    title = "US Annual Inflation by CPI Division",
    subtitle = "Focusing on core consumer expenses",
    x = "Year",
    y = "Annual Inflation Rate (%)",
    color = "CPI Division",
    shape = "CPI Division"
...
The plot reveals distinctly different behavior depending on the economic sector. Categories like Education and Health generally display a steady, persistent rate of positive inflation across the timeline without huge spikes. In contrast, Transport demonstrate far more volatility. This aligns well with general knowledge, as the volatile prices of gas and real estate fluctuate heavily with the broader economy, whereas costs like college tuition simply trend upward over time.
# Wrap-up
